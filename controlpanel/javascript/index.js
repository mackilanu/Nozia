...
function change_files_success(response) {
    ...
    if(response.status == "OK"){
	var uploades_left = 3 - parseInt(response.file.length);
	s += 'Uppladdningar kvar:'+ uploades_left;
	...
	s += '</tbody>';
	s += '</table>';
	s += '<button class="btn btn-success" onclick="add_file()"';
	if(uploades_left == 0)
	    s += 'disabled';
	s += '>Ladda upp fil</button>';
...
function add_file_success(response){
	...
	alert("Du har redan laddat upp max antal filer!");
    }
...
function remove_file(value) {
    ...
    var check = confirm("Vill du verkligen radera filen?");
    ...
    $.getJSON("ajax/delete_file.php", objekt)
        .done(function(data) {
            remove_file_success(data);
        })
        .fail(function() {
            remove_file_error();
        })
...
function remove_file_success(response) {
    if(response.status == "OK"){
	change_files();
    }
    if(response.status == "Error"){
