...
function RegisterCompany($Category, $CS, $Name, $Password, $email, $mailVerify, $Username, $orgnr, $Phone, $Adress){
    ...
    list($num_rows, $result) = opendb("", $SQL);
     ...
     if(CompanyPage($email, $Adress, $Phone) == true && CompanyPost($email) == "OK" )
      return '{"status": "OK"}';

     return '{"status": "Partly"}';
}

function CompanyPost($email){
 
   
	$SQL = "CALL read_CompanyEmail('$email')";
    $message_data = "SQL-ERROR AT foretag/ajax/register.php function CompanyPost($email)";
    ...
    $SQL = "CALL insert_Company_post('". $row['ID'] ."', 'Välkommen till Nozia!', 'Här kan ditt företag ange information om kommande händelser inom företaget, eller varför inte ett välkomstmedelande?')";

    list($affected_rows, $result) = opendb($message_data, $SQL);

    if($affected_rows == 0 or !$result) return 'Error';
